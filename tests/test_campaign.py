...
def _mk_sub(tag: str) -> "Submission":
    ...
    ``all_messages()`` yields 2 items (gpt_oss then gemma_4) -- most callers
    (``make_record``, the scoring helpers) don't need per-message alignment at all.
    """
    from jed_attack.campaign.submission import Message, MessageType, Submission
...
def test_portfolio_lambda_stamps_distinct_objective_scheme() -> None:
    """A non-zero portfolio weight earns its own scheme tag/pool."""
    from jed_attack.campaign import blackboard, config

    assert blackboard.objective_scheme_name(0.0) == "optimal_pareto_v26"
    assert blackboard.objective_scheme_name(2.0) == "portfolio2_optimal_pareto_v26"
    # OBJECTIVE_NAME reflects the live weight (portfolio diversity is on by default).
...
def test_objective_scheme_encodes_gate_guardrail_v26() -> None:
    ...
    assert bb.objective_scheme_name(0.0) == "optimal_pareto_v26"
    # OBJECTIVE_NAME carries the live weight (portfolio diversity is on by default),
    # but its base always encodes the gate guardrail + v26.
    assert bb.OBJECTIVE_NAME == bb.objective_scheme_name(config.PORTFOLIO_LAMBDA)
